...
def process_one(row: dict, engine: TradePlanEngine, start_date: str, end_date: str) -> dict | None:
    symbol = row["symbol"]
    name = row["name"]
    try:
        ...
        if daily is None or daily.empty or len(daily) < 60:
            return None
        snapshot = build_snapshot(WatchSymbol(symbol=symbol, name=name, tags=[]), daily, ACTIVE_MARKET_VALUE_PCT)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        futures = [executor.submit(process_one, row, engine, start_date, end_date) for row in records]
    ...
    write_html(rows, stats, daily_html)
